...
function subscribeUser(id, data) {
  ...
  return axios
    .post(SUBSCRIBE_URL, { userId: id, courseId })
    .then(user => user)
    .catch(err => ({}));
}
...
function unsubscribeUser(id, data) {
  ...
  return axios
    .post(UNSUBSCRIBE_URL, { userId: id, courseId })
    .then(user => user)
    .catch(err => ({}));
}
...
function callAction({ action, userId, text }) {
  return axios
    .post(CALL_ACTION_URL, Object.assign({ action, userId, text }))
    .then(user => user)
    .catch(err => ({}));
}
